...
def start_message(message):
    ...
    bot.send_message(chat_id,'Welcome my Dear Guest!', reply_markup=inline_keyboard)
    ...
    if message.text.isdigit():
        ...
        if attempt < 8:
            ...
            if n < number:
                ...
                bot.send_sticker(message.chat.id, sti)
                msg = bot.send_message(message.chat.id, 'Эу ты что? дай немного газу. Число больше.')
                bot.register_next_step_handler(msg, lambda message: random_number(message, attempt))

            elif n > number:
                sti = open('./sticker3.webp','rb')
                bot.send_sticker(message.chat.id, sti)
                msg = bot.send_message(message.chat.id, 'Воу воу, полегче чуть поменьше число давай)')
                bot.register_next_step_handler(msg, lambda message: random_number(message, attempt))
            else:
                sti = open('./sticker4.webp','rb')
                bot.send_sticker(message.chat.id, sti)
                bot.send_message(message.chat.id, 'Поздравляем вас, с победой. Сегодня вы достойны апплодисментов!!!'.format(attempt - 1))
        else:
